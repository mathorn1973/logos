...
`modal-foundation-1` may formalize only the modal foundation needed to state and test later ontological systems. It must not introduce:
...
A result may be described as a LOGOS theorem only after:
...
5. the semantic reading has been independently reviewed;
6. the result is classified as theorem, countermodel, consistency witness, or interpretation.
